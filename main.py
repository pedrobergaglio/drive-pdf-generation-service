...
def generate_pdf(data, output_path):
    ...
    pdf.cell(0, 10, f"Pago a {data['condicion_pago']} días", ln=True)
    ...
    for producto in data['productos_pedidos']:
        ...
        pdf.cell(15, 5, str(producto['cantidad']), border=0)
        pdf.cell(25, 5, str(producto['product_id']), border=0)
        pdf.multi_cell(100, 5, producto['product'])
        y_offset = pdf.get_y()
        pdf.set_xy(x_productos + 140, y_offset - 5)
        pdf.cell(30, 5, f"N° serie {producto['n_serie']}", border=0)
        y_offset = pdf.get_y() + 5  # Espacio después de cada producto
